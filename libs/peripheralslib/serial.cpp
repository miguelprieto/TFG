...
void UART_1::set_baud_rate(int baud_rate, bool ubrgh)
{
    U1BRG = baud_rate; 		  // BAUD Rate Setting
    U1MODEbits.BRGH  = ubrgh;
}


#ifndef UART1_DISABLE_INTERRUPT
...
void UART_2::set_baud_rate(int baud_rate, bool ubrgh)
{
    U2BRG = baud_rate; 		  // BAUD Rate Setting
    U2MODEbits.BRGH  = ubrgh;
}
...
extern "C" void __attribute__((__interrupt__,__no_auto_psv__)) _U2RXInterrupt(void)
{
    char rxchar ;
    if ( U2STAbits.FERR ) {
        U2STAbits.FERR = 0; // clear framing error if it occurs
    ...
    rxchar = U2RXREG ;
    if (uart2_ptr->m_rx_handler != NULL)
        uart2_ptr->m_rx_handler(rxchar);
    IFS1bits.U2RXIF = 0 ; // clear the interrupt
}
